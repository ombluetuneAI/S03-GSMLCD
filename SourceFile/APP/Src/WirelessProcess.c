    ...
    switch(zoneType)
	{
		...
		case SystemZoneSOS://紧急报警
                ...
                if(buffer[12]==0x37)
                    SirenAlarm();
			break;
		case SystemZoneDoorbell://门铃，在布防的状态下为正常警区
...
void UnlockProcess(void)
{
	...
	else
	{
		...
		if(state)
			App.Data.CallData.CallCidMessage=ENABLE;//接通中心机
	}
...
//            System.SystemApp.ScreenSet();
      ...
      else if(!remoteNumber)return;//非法遥控直接返回
...
//				zoneID[4]=0;
//				zoneID[5]=0;
				System.Device.Storage.Parameter.StorageWriteData(addrID,(u16*)zoneID,2);//写入
